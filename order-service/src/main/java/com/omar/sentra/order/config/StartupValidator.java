...
/**
 * Rejects unsupported or unsafe configuration before traffic is accepted.
 */
@Component
public class StartupValidator implements SmartInitializingSingleton {
    ...
    public StartupValidator(Environment environment, OrderServiceProperties properties) {
     ...
     * @param properties order-service properties
     */
    public static void validate(String[] activeProfiles, OrderServiceProperties properties) {
        if (activeProfiles.length == 0) {
            throw new IllegalStateException("An explicit Spring profile is required.");
        }
        if (!Set.of("memory", "postgres").contains(properties.repository().mode())
                || !"default".equals(properties.repository().seedDataset())) {
            throw new IllegalStateException("Only documented repository modes and datasets are supported.");
        }
        durationBetween(
        ...
        durationBetween(
                properties.idempotency().cleanupInterval(),
                Duration.ofSeconds(10),
                Duration.ofHours(1),
                "IDEMPOTENCY_CLEANUP_INTERVAL");
        if (properties.limits().defaultPageSize() > properties.limits().maxPageSize()) {
            throw new IllegalStateException("DEFAULT_PAGE_SIZE cannot exceed MAX_PAGE_SIZE.");
        }
        ...
        if (!"order-service".equals(properties.serviceName())) {
            throw new IllegalStateException("SERVICE_NAME must be order-service in production-like profiles.");
        }
        if (properties.repository().seedEnabled()) {
            throw new IllegalStateException("Deterministic order seeds are forbidden in production.");
        }
        ...
        if (properties.logging().includeOrderIds()
                || properties.logging().includeOwnerReferences()) {
            throw new IllegalStateException("Production logging cannot expose order or owner references.");
        }
    ...
    private static void rejectBootDebugSwitch(Environment environment, String property) {
        ...
        if (value != null && !"false".equalsIgnoreCase(value)) {
            throw new IllegalStateException(
                    "Production cannot enable Spring Boot " + property + " logging.");
        }
    }
